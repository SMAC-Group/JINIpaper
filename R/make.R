...
makeglmrob2 <- function(formula, family = gaussian, data, coefficients, weights,
                    ...
                    contrasts = NULL, ...)
{
  ...
  ## We want to set the name on this call and the one below for the
  ## sake of messages from the fitter function
  fit <- eval(call("glmrob2.nofit", #if(is.function(method)) "method" else method,
                   coefficients = coefficients, x = X, y = Y, weights = weights, start = start,
                   etastart = etastart, mustart = mustart,
                   offset = offset, family = family, control = control,
  ...
  if(length(offset) && attr(mt, "intercept") > 0L) {
    fit2 <-
      eval(call("glmrob2.nofit", coefficients = coefficients, #if(is.function(method)) "method" else method,
                x = X[, "(Intercept)", drop=FALSE], y = Y,
                weights = weights, offset = offset, family = family,
                control = control, intercept = TRUE))
    ## That fit might not have converged ....
...
#' @importFrom glmrob2 glmrob2.control
glmrob2.nofit <- function (x, y, coefficients, weights = rep(1, nobs), start = NULL,
...
#' @param robust use robust estimator
#' @importFrom stats rbinom
#' @seealso \code{\link[stats]{glm}}, \code{\link[ib]{simulation}}
#' @export
make_logistic <- function(x, coefficients, robust = FALSE){
  if(ncol(x) != length(coefficients) - 1L) stop("Design matrix mistmaches coefficients size")
  y_tmp <- rbinom(nrow(x),1,0.5)
  if(!robust) object <- makeglm(y_tmp ~ x, family = binomial(link = "logit"), coefficients = coefficients) else 
    object <- makeglmrob2(y_tmp ~ x, family = binomial(link = "logit"), coefficients = coefficients)
  object
}
