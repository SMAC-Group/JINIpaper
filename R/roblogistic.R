...
r_logistic_misc <- function(thetas, x, seed, FN, FP){
  set.seed(seed)
  n <- nrow(x)
  p <- ncol(x)
...
#' @param seed for random number generator
#' @export
robmisclogisticWmle1_ib <- function(x, thetastart, c = 4.685061, H = 200, maxit=200, tol=1e-7, verbose=FALSE, seed=321, FP=0.0, FN=0.0){
  p <- length(thetastart)
  pi0 <- t0 <- thetastart
