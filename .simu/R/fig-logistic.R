...
for(i in 1:n_setting){
  load(paste0(".simu/data/logistic_1_setting_",i,".rds"))
...
allplot(results = super_res, index = ind, theta = theta0, lab_par = lab_beta, 
        ...
        path = paste0(getwd(),"/.simu/"), extension = "logistic_inconsistent", p = p, y_lim=y_lim)
...
super_res <- list(setting1 = array(dim=c(MC,p[1],n_methods)),
                  ...
                  setting3 = array(dim=c(MC,p[3],n_methods)))
for(i in 1:n_setting){
  load(paste0(".simu/data/logistic_2_setting_",i,".rds"))
...
allplot(results = super_res, index = ind, theta = theta0, lab_par = lab_beta, 
        lab_mth = lab_methods, lab_set = lab_main, cols = cols, pchs = pchs, 
        path = paste0(getwd(),"/.simu/"), extension = "logistic_consistent", p = p, y_lim=y_lim)
